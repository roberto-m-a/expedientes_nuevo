...
var _alphabetSearch;
//Funcion que permite retornar los registros que cumplan con el filtro de apellidos
...
//Funcion para poder inicializar la tabla de los documentos
$(document).ready(function () {
    var table = $('#tablaDocs').DataTable({
        ...
        columns: [
            ...
            {
                title: "Entregada", data: null, render: function (data, type, row, meta) {
            ...
            { title: 'Periodo', data: 'nombre_corto' },
            {
                title: "Acciones", data: null, render: function (data, type, row, meta) {
                    return (data.IdUsuario == props.user.id) ?
    ...
    $('<span id="clear" class="clear active"/>')
        .data('letter', '')
        .html('Todos')
        .appendTo(alphabet);
...
//Metodo para validar y editar la informacion del archivo
    ...
    formEdit.post(route('validar.entrega'), {
        onSuccess: () =>{
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Swal.fire('Entrega confirmada', 'El código es correcto.', 'success');
...
//Metodo para manejar los botones de la vista
onMounted(() => {
